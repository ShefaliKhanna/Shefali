...
app.post('/add.html',function(req,res){
  ...
  connection.query("INSERT INTO account (fname, sname , email , email2, pass ) values ('"+firstname+"', '"+lastname+"','"+emailid+"','"+emailid+"','"+password+"')");
  
  connection.query('SELECT * from account limit 2', function(err, rows, fields)   
{  
  if (err) throw err;  
